...
from spinup2.utils.mpi_tools import broadcast
...
def assign_params_from_flat(x, params):
    ...
    new_params = [tf.reshape(p_new, p.shape) for p, p_new in izip(params, splits)]
    return tf.group([tf.assign(p, p_new) for p, p_new in izip(params, new_params)])
...
def sync_all_params():
    u"""Sync all tf variables across MPI processes."""
...
class MpiAdamOptimizer(tf.compat.v1.train.AdamOptimizer):
    ...
    def compute_gradients(self, loss, var_list, **kwargs):
        ...
        """
        grads_and_vars = super(MpiAdamOptimizer, self).compute_gradients(loss, var_list, **kwargs)
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        ...
        """
        opt = super(MpiAdamOptimizer, self).apply_gradients(grads_and_vars, global_step, name)
